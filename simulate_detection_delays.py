...
from batch_selection import BatchSelectorOnAutocorrelation, find_nearest
...
def generate_random_change_point_time(arl_0, end_of_warm_up_time = None):
    """
...
def simulate_detection_delays_using_r_cpm(data_df: pd.DataFrame, rho: List[float], delta_rhos: List[float],
    ...
    for run_idx in range(num_runs):
        ...
        for delta_rho in delta_rhos:
            ...
            if batch_size:
                ...
                r_estimated_changepoint_index = cpm_func.Detect_r_cpm_NonParametricChangePoint(wait_times)
                ...
                if estimated_changepoint_idx > 0:
                    ...
                    if batch_size:
                        try:
                            ...
                            print(f"Estimated changepoint idx {estimated_changepoint_idx} vs. array of length {len(batch_centers)}")
                            raise
                    else:
    ...
    for delta_rho in delta_rhos:
        ...
        prob_detect = true_positive[delta_rho] / float(num_runs)
...
def simulate_detection_delay_by_rho_delta_rho_specific_batch_size(data_df, rho, delta_rhos, arr_rate_0, num_runs,
    ...
    for run_idx in range(num_runs):
        ...
        for delta_rho in delta_rhos:
            arr_rate_1 = arr_rate_0 * (1 + delta_rho)
            my_arrival_rates = [arr_rate_0, arr_rate_1]
            time_of_change = generate_random_change_point_time(end_time)
            time_of_changes = [-1, time_of_change]
...
def main_simple_rcpm(batch_size, is_parametric=True):
    """
    ...
    data_store, false_positive_list = simulate_detection_delays_using_r_cpm(data_store, rho, delta_rho, arr_rate_0,
                                                                            num_runs, start_time, end_time,
    ...
    if is_parametric:
        data_store.to_csv(f"./Results/GLRT_ROSS/Performance_Tests/SimpleDetection_Batch_of_size_{batch_size}_rcpm.csv",
                          index=False)
    else:
        if batch_size is None:
            data_store.to_csv("./Results/GLRT_ROSS/Performance_Tests/SimpleDetection_Batch_of_size_nonparametric_rcpm.csv",
                              index=False)
        else:
            data_store.to_csv(
                f"./Results/GLRT_ROSS/Performance_Tests/SimpleDetection_Batch_of_size_{batch_size}_rcpm_nonparam.csv",
                index=False)
...
if __name__ == "__main__":
    ...
    # main_3()
    #main_simple_roc()
    main_simple_rcpm(50, is_parametric=False)
    main_simple_rcpm(100, is_parametric=False)
    main_simple_rcpm(150, is_parametric=False)
    main_simple_rcpm(200, is_parametric=False)
    main_simple_rcpm(500, is_parametric=False)
    main_simple_rcpm(1000, is_parametric=False)
    main_simple_rcpm(50)
    main_simple_rcpm(100)
    main_simple_rcpm(150)
    main_simple_rcpm(200)
    main_simple_rcpm(500)
    main_simple_rcpm(1000, is_parametric=True)
