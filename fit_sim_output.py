...
class LogLogisticFit(FitClass):
    ...
    def log_logistic_error_function_beta_gradient(self, alpha, beta):
        ...
        for x, y in zip(self._x, self._y):
            ...
            return_value += 2 * first_part * (
                    ((1 + beta * np.log(x / alpha)) * 1 / alpha * math.pow((x / alpha), (beta - 1)))
...
def plot_predicted_gaussian_process(sim_param, X, y, best_gp, x_mode, num_points=20):
    ...
    plt.savefig(
        "Figures/BatchSizeFit/PassingThroughZero/batch_size_config_rho_({})_delta_rho_({})_tc_{}_gp_fit.png".format(
            sim_param.rho,
            sim_param.delta_rho,
            sim_param.tc))
    plt.legend()
